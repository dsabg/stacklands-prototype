...
public class Card : MonoBehaviour
{
    ...
    /// </summary>
    public const float PickupHeight = 0.55f;
    /// <summary>
    ...
    public void Initialise(CardData data)
    {
        ...
        Quantity = 1;
        DescriptionText.text = Data.Describe;
        ...
        switch (Rarity)
        {
            ...
            case CardRarity.Uncommon:
                _Border.color = Color.cyan;
                break;
            case CardRarity.Rare:
                _Border.color = new Color(0.5f, 0f, 1f);
                break;
            case CardRarity.Legendary:
                _Border.color = new Color(1f, 0.647f, 0f);
                break;
        ...
        else if (data.CardClass == CardClass.People)
        {
            Sellable = false;
        ...
        else if (data.CardClass == CardClass.Weapon)
        {
            Sellable = true;
            _PriceImage.gameObject.SetActive(true);
            _PriceText.text = data.Value.ToString();
        ...
        else if(data.CardClass == CardClass.Money)
        {
            Sellable=false;
            _PriceImage.gameObject.SetActive(true);
            _PriceText.text = data.Value.ToString();
    ...
    {
        SetDraggable(false);
        SetInteractable(InteractorState.None);
        SetActive(true);
        IsBeingDestroyed = true;
    }

    public void SetInteractable(InteractorState state)
    {
    ...
    public void MoveToStackedCard()
    {
        ...
        if (StackNode.Previous != null && StackNode.Previous.Value.MultiCard != null) offset.y *= 2;
    ...
    {
        Unstack();
        _ = MoveComponent.MoveZ(-StackZOffset - PickupHeight);//向z轴平滑移动
    }
